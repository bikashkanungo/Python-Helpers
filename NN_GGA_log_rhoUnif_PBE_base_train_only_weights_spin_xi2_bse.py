...
class DatMolecules(Dataset):
    def __init__(self, moleculesMetaData):
        ...
        for index in range(len(moleculesMetaData)):
            ...
            rhoInpDataStack = np.column_stack((rho, drho.reshape(npoints,3*nSpin)))
            rhoInpData = torch.from_numpy(rhoInpDataStack).to(device)
            self.rhoInpDataTensor.append(rhoInpData)
            ...
            self.ddrhoDataTensor.append(torch.from_numpy(ddrho).to(device))
    ...
    def __getitem__(self, index):
        return {'rhoInpData': self.rhoInpDataTensor[index], 
                ...
                'ddrhoData': self.ddrhoDataTensor[index],
...
class DNN(Module):
    ...
    def forward(self, rho_b):
        ...
        modGradRhoTotal = (((rho_b[:,2:5]+rho_b[:,5:8])**2.0).sum(dim=1))**0.5
        Cs = 1.0/(2.0*(3.0*math.pi**2.0)**(1.0/3.0))
        s = Cs*modGradRhoTotal/(rhoTotal**(4.0/3.0))
    ...
    def getVxc(self, rho, ddrho):
        ...
        drho = torch.zeros(nx,nspin,3).to(device)
        for iSpin in range(nspin):
            index = nspin + iSpin*3
            drho[:,iSpin] = (rho[:,index:index+3]).detach().clone()
        ...
        for iSpin in range(nspin):
            ...
            for jSpin in range(nspin):
               t1 = grad(gdrho, rho, drho[:,jSpin], retain_graph=True)[0]
               vxc[:,iSpin] = vxc[:,iSpin] - t1[:,jSpin]
               t21 = grad(gdrho, rho, ddrho[:,jSpin,0], retain_graph=True)[0]
               t22 = grad(gdrho, rho, ddrho[:,jSpin,1], retain_graph=True)[0]
               t23 = grad(gdrho, rho, ddrho[:,jSpin,2], retain_graph=True)[0]
               id2 = nspin+3*jSpin
...
def loss_batch(model, loss_func, molecules_b, opt = None):
    ...
    ddrhoData_b = molecules_b['ddrhoData']
    ...
    for Id in range(numMols):
        ...
        vxc_loss_mol = 0.0
        vxcL2Square = 0.0 
        for rhoInp_b, ddrho_b, vxc_ref_b, quad_b in mol_data_loader:
        ...
        for rhoInp_b, ddrho_b, vxc_ref_b, quad_b in mol_data_loader:
            ...
            vxc_b = model.getVxc(rhoInp_b, ddrho_b) + vxcbase_b
            rho_b_copy = (rhoInp_b[:,0:2]).detach().clone() 
